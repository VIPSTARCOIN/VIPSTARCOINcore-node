...
var VIPSTARCOINRPC = require('VIPSTARCOINd-rpc');
...
/**
 * Provides a friendly event driven API to VIPSTARCOINd in Node.js. Manages starting and
 * stopping VIPSTARCOINd as a child process for application support, as well as connecting
 * to multiple VIPSTARCOINd processes for server infrastructure. Results are cached in an
 * LRU cache for improved performance and methods added for common queries.
 *
 * @param {Object} options
 * @param {Node} options.node - A reference to the node
 */
function VIPSTARCOIN(options) {
  if (!(this instanceof VIPSTARCOIN)) {
    return new VIPSTARCOIN(options);
  }
...
util.inherits(VIPSTARCOIN, Service);

VIPSTARCOIN.dependencies = [];

VIPSTARCOIN.DEFAULT_MAX_TXIDS = 1000;
VIPSTARCOIN.DEFAULT_MAX_HISTORY = 50;
VIPSTARCOIN.DEFAULT_SHUTDOWN_TIMEOUT = 15000;
VIPSTARCOIN.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS = 0.9999;
VIPSTARCOIN.DEFAULT_MAX_ADDRESSES_QUERY = 10000;
VIPSTARCOIN.DEFAULT_SPAWN_RESTART_TIME = 5000;
VIPSTARCOIN.DEFAULT_SPAWN_STOP_TIME = 10000;
VIPSTARCOIN.DEFAULT_TRY_ALL_INTERVAL = 1000;
VIPSTARCOIN.DEFAULT_REINDEX_INTERVAL = 10000;
VIPSTARCOIN.DEFAULT_START_RETRY_INTERVAL = 5000;
VIPSTARCOIN.DEFAULT_TIP_UPDATE_INTERVAL = 15000;
VIPSTARCOIN.DEFAULT_TRANSACTION_CONCURRENCY = 5;
VIPSTARCOIN.DEFAULT_CONFIG_SETTINGS = {
  server: 1,
  whitelist: '127.0.0.1',
  txindex: 1,
  addressindex: 1,
...
VIPSTARCOIN.prototype._initDefaults = function(options) {
  /* jshint maxcomplexity: 15 */

  // limits
  this.maxTxids = options.maxTxids || VIPSTARCOIN.DEFAULT_MAX_TXIDS;
  this.maxTransactionHistory = options.maxTransactionHistory || VIPSTARCOIN.DEFAULT_MAX_HISTORY;
  this.maxAddressesQuery = options.maxAddressesQuery || VIPSTARCOIN.DEFAULT_MAX_ADDRESSES_QUERY;
  this.shutdownTimeout = options.shutdownTimeout || VIPSTARCOIN.DEFAULT_SHUTDOWN_TIMEOUT;

  // spawn restart setting
  this.spawnRestartTime = options.spawnRestartTime || VIPSTARCOIN.DEFAULT_SPAWN_RESTART_TIME;
  this.spawnStopTime = options.spawnStopTime || VIPSTARCOIN.DEFAULT_SPAWN_STOP_TIME;

  // try all interval
  this.tryAllInterval = options.tryAllInterval || VIPSTARCOIN.DEFAULT_TRY_ALL_INTERVAL;
  this.startRetryInterval = options.startRetryInterval || VIPSTARCOIN.DEFAULT_START_RETRY_INTERVAL;

  // rpc limits
  this.transactionConcurrency = options.transactionConcurrency || VIPSTARCOIN.DEFAULT_TRANSACTION_CONCURRENCY;

  // sync progress level when zmq subscribes to events
  this.zmqSubscribeProgress = options.zmqSubscribeProgress || VIPSTARCOIN.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS;
};

VIPSTARCOIN.prototype._initCaches = function() {
...
VIPSTARCOIN.prototype._initClients = function() {
  var self = this;
  this.nodes = [];
  this.nodesIndex = 0;
 ...
 */
VIPSTARCOIN.prototype.getAPIMethods = function() {
  var methods = [
 ...
 */
VIPSTARCOIN.prototype.getPublishEvents = function() {
  return [
    {
...
VIPSTARCOIN.prototype.subscribe = function(name, emitter) {
  this.subscriptions[name].push(emitter);
  log.info(emitter.remoteAddress, 'subscribe:', 'VIPSTARCOINd/' + name, 'total:', this.subscriptions[name].length);
};

VIPSTARCOIN.prototype.unsubscribe = function(name, emitter) {
  var index = this.subscriptions[name].indexOf(emitter);
  if (index > -1) {
    this.subscriptions[name].splice(index, 1);
  }
  log.info(emitter.remoteAddress, 'unsubscribe:', 'VIPSTARCOINd/' + name, 'total:', this.subscriptions[name].length);
};


VIPSTARCOIN.prototype.subscribeBalance = function(emitter, addresses) {
    var self = this;
...
VIPSTARCOIN.prototype.unsubscribeBalance = function(emitter, addresses) {
...
VIPSTARCOIN.prototype.unsubscribeBalanceAll = function(emitter) {
...
VIPSTARCOIN.prototype.subscribeAddress = function(emitter, addresses) {
  var self = this;
  ...
  log.info(emitter.remoteAddress, 'subscribe:', 'VIPSTARCOINd/addresstxid', 'total:', _.size(this.subscriptions.address));
};

VIPSTARCOIN.prototype.unsubscribeAddress = function(emitter, addresses) {
  var self = this;
  if(!addresses) {
  ...
  log.info(emitter.remoteAddress, 'unsubscribe:', 'VIPSTARCOINd/addresstxid', 'total:', _.size(this.subscriptions.address));
};
 ...
 */
VIPSTARCOIN.prototype.unsubscribeAddressAll = function(emitter) {
...
VIPSTARCOIN.prototype._getDefaultConfig = function() {
  var config = '';
  var defaults = VIPSTARCOIN.DEFAULT_CONFIG_SETTINGS;
...
VIPSTARCOIN.prototype._parseBitcoinConf = function(configPath) {
  var options = {};
...
VIPSTARCOIN.prototype._expandRelativeDatadir = function() {
...
VIPSTARCOIN.prototype._loadSpawnConfiguration = function(node) {
  /* jshint maxstatements: 25 */

  $.checkArgument(this.options.spawn, 'Please specify "spawn" in VIPSTARCOINd config options');
  $.checkArgument(this.options.spawn.datadir, 'Please specify "spawn.datadir" in VIPSTARCOINd config options');
  $.checkArgument(this.options.spawn.exec, 'Please specify "spawn.exec" in VIPSTARCOINd config options');

  this._expandRelativeDatadir();

  var spawnOptions = this.options.spawn;
  var configPath = path.resolve(spawnOptions.datadir, './VIPSTARCOIN.conf');
  if (spawnOptions.configdir != null) {
    configPath = path.resolve(spawnOptions.configdir, './VIPSTARCOIN.conf');
  }

  log.info('Using VIPSTARCOIN config file:', configPath);
...
VIPSTARCOIN.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    spawnConfig.txindex && spawnConfig.txindex === 1,
    '"txindex" option is required in order to use transaction query features of VIPSTARCOINcore-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of VIPSTARCOINcore-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of VIPSTARCOINcore-node. ' +
  ...
  $.checkState(
    ...
    '"server" option is required to communicate to VIPSTARCOINd from bitcore. ' +
      'Please add "server=1" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubrawtx,
    '"zmqpubrawtx" option is required to get event updates from VIPSTARCOINd. ' +
      'Please add "zmqpubrawtx=tcp://127.0.0.1:<port>" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubhashblock,
    '"zmqpubhashblock" option is required to get event updates from VIPSTARCOINd. ' +
  ...
  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that VIPSTARCOINd is undergoing a reindex. ' +
             ...
             'of VIPSTARCOINcore-node services will start.');
    node._reindex = true;
  }
};

VIPSTARCOIN.prototype._resetCaches = function() {
...
VIPSTARCOIN.prototype._tryAllClients = function(func, callback) {
  var self = this;
...
VIPSTARCOIN.prototype._wrapRPCError = function(errObj) {
  var err = new errors.RPCError(errObj.message);
  err.code = errObj.code;
  return err;
};

VIPSTARCOIN.prototype._initChain = function(callback) {
  var self = this;
    ...
    self.client.getBlock(response.result, function(err, response) {
      ...
      self.client.getBlockHash(0, function(err, response) {
        ...
        self.getRawBlock(blockhash, function(err, blockBuffer) {
          ...
          self.emit('ready');
          log.info('VIPSTARCOIN Daemon Ready');
          callback();
        });
      });
...
VIPSTARCOIN.prototype._getDefaultConf = function() {
  var networkOptions = {
    rpcport: 31916
  };
...
VIPSTARCOIN.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === bitcore.Networks.testnet) {
    networkPath = 'testnet3/VIPSTARCOIN.conf';
    if (this.node.network.regtestEnabled) {
      networkPath = 'regtest/VIPSTARCOIN.conf';
    }
  }
  return networkPath;
};

VIPSTARCOIN.prototype._getNetworkOption = function() {
  var networkOption;
  if (this.node.network === bitcore.Networks.testnet) {
...
VIPSTARCOIN.prototype._zmqBlockHandler = function(node, message) {
  var self = this;
  ...
  if (!self.zmqKnownBlocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.hashblock.length; i++) {
      this.subscriptions.hashblock[i].emit('VIPSTARCOINd/hashblock', message.toString('hex'));
    }
  }

};

VIPSTARCOIN.prototype._rapidProtectedUpdateTip = function(node, message) {
  var self = this;
...
VIPSTARCOIN.prototype._updateTip = function(node, message) {
  var self = this;
    ...
    if(!self.node.stopping) {
      self.syncPercentage(function(err, percentage) {
        if (err) {
          ...
          log.info('VIPSTARCOIN Height:', self.height, 'Percentage:', percentage.toFixed(2));
        }
      });
    }
  }
...
VIPSTARCOIN.prototype._getAddressesFromTransaction = function(transaction) {
  var addresses = [];
...
VIPSTARCOIN.prototype._notifyBalanceSubscribers = function(txid, transaction) {
...
VIPSTARCOIN.prototype._notifyBalanceSubscriber = function (address, txid) {
    var self = this;
...
VIPSTARCOIN.prototype._notifyAddressTxidSubscribers = function(txid, transaction) {
  var addresses = this._getAddressesFromTransaction(transaction);
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
    if(this.subscriptions.address[address]) {
      ...
      for(var j = 0; j < emitters.length; j++) {
        emitters[j].emit('VIPSTARCOINd/addresstxid', {
          address: address,
          txid: txid
        });
      }
...
VIPSTARCOIN.prototype._zmqTransactionHandler = function(node, message) {
  ...
  if (!self.zmqKnownTransactions.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.rawtransaction.length; i++) {
      this.subscriptions.rawtransaction[i].emit('VIPSTARCOINd/rawtransaction', message.toString('hex'));
    }
...
VIPSTARCOIN.prototype._checkSyncedAndSubscribeZmqEvents = function(node) {
  var self = this;
  var interval;
  ...
  checkAndSubscribe(function(err, synced) {
    ...
    if (!synced) {
      ...
      }, node._tipUpdateInterval || VIPSTARCOIN.DEFAULT_TIP_UPDATE_INTERVAL);
    }
  });

};

VIPSTARCOIN.prototype._subscribeZmqEvents = function(node) {
  var self = this;
...
VIPSTARCOIN.prototype._initZmqSubSocket = function(node, zmqUrl) {
...
VIPSTARCOIN.prototype._checkReindex = function(node, callback) {
  var self = this;
  var interval;
  function finish(err) {
    clearInterval(interval);
  ...
  if (node._reindex) {
    interval = setInterval(function() {
      node.client.getBlockchainInfo(function(err, response) {
        ...
        log.info('VIPSTARCOIN Core Daemon Reindex Percentage: ' + percentSynced.toFixed(2));
    ...
    }, node._reindexWait || VIPSTARCOIN.DEFAULT_REINDEX_INTERVAL);
  } else {
    callback();
  }
};

VIPSTARCOIN.prototype._loadTipFromNode = function(node, callback) {
  var self = this;
  node.client.getBestBlockHash(function(err, response) {
    if (err && err.code === -28) {
...
VIPSTARCOIN.prototype._stopSpawnedBitcoin = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  var pidPath = spawnOptions.datadir + '/VIPSTARCOINd.pid';
...
VIPSTARCOIN.prototype._spawnChildProcess = function(callback) {
  var self = this;
  ...
  self._stopSpawnedBitcoin(function(err) {
    ...
    log.info('Starting VIPSTARCOIN process');
    ...
    self.spawn.process.once('exit', function(code) {
      if (!self.node.stopping) {
        log.warn('VIPSTARCOIN process unexpectedly exited with code:', code);
        log.warn('Restarting VIPSTARCOIN child process in ' + self.spawnRestartTime + 'ms');
        setTimeout(function() {
          self._spawnChildProcess(function(err) {
            if (err) {
              return self.emit('error', err);
            }
            log.warn('VIPSTARCOIN process restarted');
          });
    ...
    async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
      ...
      node.client = new VIPSTARCOINRPC({
        protocol: 'http',
        host: '127.0.0.1',
      ...
      if (exitShutdown) {
        return callback(new Error('Stopping while trying to spawn VIPSTARCOINd.'));
      }
...
VIPSTARCOIN.prototype._connectProcess = function(config, callback) {
  var self = this;
  var node = {};
  var exitShutdown = false;
  ...
  async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
    ...
    if (exitShutdown) {
      return callback(new Error('Stopping while trying to connect to VIPSTARCOINd.'));
    }
 ...
 */
VIPSTARCOIN.prototype.start = function(callback) {
  var self = this;
  ...
  async.series([
    ...
    if (self.nodes.length === 0) {
      return callback(new Error('VIPSTARCOIN configuration options "spawn" or "connect" are expected'));
    }
 ...
 * @param {Function} callback
 */
VIPSTARCOIN.prototype.isSynced = function(callback) {
 ...
 * @param {Function} callback
 */
VIPSTARCOIN.prototype.syncPercentage = function(callback) {
  var self = this;
...
VIPSTARCOIN.prototype._normalizeAddressArg = function(addressArg) {
 ...
 * @param {String|Address|Array} addressArg - An address string, VIPSTARCOINcore address, or array of addresses
 * @param {Object} options
 * @param {Function} callback
 */
VIPSTARCOIN.prototype.getAddressBalance = function(addressArg, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
VIPSTARCOIN.prototype.getAddressUnspentOutputs = function(addressArg, options, callback) {
  var self = this;
...
VIPSTARCOIN.prototype._getBalanceFromMempool = function(deltas) {
  var satoshis = 0;
...
VIPSTARCOIN.prototype._getTxidsFromMempool = function(deltas) {
  var mempoolTxids = [];
...
VIPSTARCOIN.prototype._getHeightRangeQuery = function(options, clone) {
  if (options.start >= 0 && options.end >= 0) {
    if (options.end > options.start) {
      throw new TypeError('"end" is expected to be less than or equal to "start"');
    }
    if (clone) {
      // reverse start and end as the order in VIPSTARCOINcore is most recent to less recent
 ...
 * @param {Function} callback
 */
VIPSTARCOIN.prototype.getAddressTxids = function(addressArg, options, callback) {
  /* jshint maxstatements: 20 */
  var self = this;
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
...
VIPSTARCOIN.prototype._getConfirmationsDetail = function(transaction) {
...
VIPSTARCOIN.prototype._getAddressDetailsForInput = function(input, inputIndex, result, addressStrings) {
  if (!input.address) {
    return;
  }
...
VIPSTARCOIN.prototype._getAddressDetailsForOutput = function(output, outputIndex, result, addressStrings) {
  if (!output.address) {
    return;
  }
...
VIPSTARCOIN.prototype._getAddressDetailsForTransaction = function(transaction, addressStrings) {
  var result = {
    addresses: {},
    satoshis: 0
  };
 ...
 * @param {Object} txid - A VIPSTARCOIN transaction id
 * @param {Function} callback
 */
VIPSTARCOIN.prototype._getAddressDetailedTransaction = function(txid, options, next) {
  var self = this;
...
VIPSTARCOIN.prototype._getAddressStrings = function(addresses) {
  var addressStrings = [];
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
...
VIPSTARCOIN.prototype._paginateTxids = function(fullTxids, fromArg, toArg) {
  var txids;
 ...
 * @param {Function} callback
 */
VIPSTARCOIN.prototype.getAddressHistory = function(addressArg, options, callback) {
  var self = this;
  var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
VIPSTARCOIN.prototype.getAddressesMempoolBalance = function(addressArg, options, callback) {
    var self = this;
    var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
VIPSTARCOIN.prototype.getAddressSummary = function(addressArg, options, callback) {
  var self = this;
  var summary = {};
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
  var summaryTxids = [];
...
VIPSTARCOIN.prototype._maybeGetBlockHash = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
VIPSTARCOIN.prototype.getRawBlock = function(blockArg, callback) {
 ...
 * @param {Function} callback
 */
VIPSTARCOIN.prototype.getBlockOverview = function(blockArg, callback) {
  var self = this;
...
VIPSTARCOIN.prototype.getJsonBlock = function(blockArg, callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
VIPSTARCOIN.prototype.getBlock = function(blockArg, callback) {
  // TODO apply performance patch to the RPC method for raw data
  var self = this;
 ...
 * @param {Function} callback
 */
VIPSTARCOIN.prototype.getBlockHashesByTimestamp = function(high, low, options, callback) {
 ...
 * @param {Function} callback
 */
VIPSTARCOIN.prototype.getBlockHeader = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
VIPSTARCOIN.prototype.estimateFee = function(blocks, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
VIPSTARCOIN.prototype.sendTransaction = function(tx, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
VIPSTARCOIN.prototype.getRawTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
VIPSTARCOIN.prototype.getJsonRawTransaction = function(txid, callback) {
  var self = this;
...
/**
 * Will get a transaction as a VIPSTARCOINcore Transaction. Results include the mempool.
 * @param {String} txid - The transaction hash
 * @param {Boolean} queryMempool - Include the mempool
 * @param {Function} callback
 */
VIPSTARCOIN.prototype.getTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
VIPSTARCOIN.prototype.getDetailedTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
VIPSTARCOIN.prototype.getBestBlockHash = function(callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
VIPSTARCOIN.prototype.getSpentInfo = function(options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
VIPSTARCOIN.prototype.getInfo = function(callback) {
...
VIPSTARCOIN.prototype.generateBlock = function(num, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
VIPSTARCOIN.prototype.stop = function(callback) {
  if (this.spawn && this.spawn.process) {
    ...
    this.spawn.process.once('exit', function(code) {
      if (!exited) {
        exited = true;
        if (code !== 0) {
          var error = new Error('VIPSTARCOINd spawned process exited with status code: ' + code);
          error.code = code;
    ...
    setTimeout(function() {
      if (!exited) {
        exited = true;
        return callback(new Error('VIPSTARCOINd process did not exit'));
      }
 ...
 * @param {Function} callback
 */
VIPSTARCOIN.prototype.listUnspent = function(minConf, maxConf, addresses, callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
VIPSTARCOIN.prototype.getNewAddress = function(callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
VIPSTARCOIN.prototype.callContract = function(address, hash, options, callback) {
    var self = this,
 ...
 * @param {Function} callback
 */
VIPSTARCOIN.prototype.getAccountInfo = function(address, callback) {
 ...
 * @param {Function} callback
 */
VIPSTARCOIN.prototype.getSubsidy = function(height, callback) {
 ...
 * @param {Function} callback
 */
VIPSTARCOIN.prototype.getTransactionReceipt = function(trxHash, callback) {
 ...
 * @param {Function} callback
 */
VIPSTARCOIN.prototype.getDgpInfo = function(callback) {
 ...
 * @return {*}
 */
VIPSTARCOIN.prototype.getMiningInfo = function(callback) {
    var self = this,
 ...
 * @return {*}
 */
VIPSTARCOIN.prototype.getStakingInfo = function(callback) {
    var self = this,
