...
/**
 * Will return the path and VIPSTARCOINcore-node configuration
...
function findConfig(cwd) {
  ...
  while (!fs.existsSync(path.resolve(directory, 'VIPSTARCOINcore-node.json'))) {
  ...
  return {
    path: directory,
    config: require(path.resolve(directory, 'VIPSTARCOINcore-node.json'))
  };
}
