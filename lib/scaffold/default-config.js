...
/**
 * Will return the path and default VIPSTARCOINcore-node configuration. It will search for the
...
function getDefaultConfig(options) {
  ...
  var defaultConfigFile = path.resolve(defaultPath, './VIPSTARCOINcore-node.json');

  if (!fs.existsSync(defaultPath)) {
    mkdirp.sync(defaultPath);
  }

  var defaultServices = ['VIPSTARCOINd', 'web'];
