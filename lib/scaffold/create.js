...
var BASE_PACKAGE = {
  ...
  dependencies: {
    ...
    'VIPSTARCOIN-lib': 'git://github.com/VIPSTARCOIN/VIPSTARCOIN-lib.git#master',
    'VIPSTARCOINcore-node': 'git://github.com/VIPSTARCOIN/VIPSTARCOINcore-node.git'
  }
};
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/VIPSTARCOINcore-node.json', configJSON);
      if (!isGlobal) {
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the VIPSTARCOINcore-node directory and configuration
