...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 'A reindex for VIPSTARCOINd is necessary for this upgrade with the "reindex=1" bitcoin.conf option. \n' +
                 'There are changes necessary in both bitcoin.conf and VIPSTARCOINcore-node.json. \n\n' +
 ...
 * @param {Object} options.config - The parsed VIPSTARCOINcore-node.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './VIPSTARCOINcore-node.json');
  ...
  node.on('ready', function() {
    log.info('VIPSTARCOINcore Node ready');
  });
...
function loadModule(req, service) {
  try {
    // first try in the built-in VIPSTARCOINcore-node services directory
    ...
    if (servicePackage.VIPSTARCOINcoreNode) {
      serviceModule = service.name + '/' + servicePackage.VIPSTARCOINcoreNode;
    }
